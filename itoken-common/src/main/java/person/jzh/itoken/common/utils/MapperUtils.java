...
//                    }
    ...
    /**
     * 根据实体类，转成条件构造器(存在数字的字段不要使用该方法)
     *
     * @param t
     * @param <T>
     * @return
     */
    public static <T> QueryWrapper objToQueryWrapper(T t) {
        ...
        try {
            ...
            for (PropertyDescriptor property : propertyDescriptors) {
                ...
                if (!"class".equals(key)) {
                    ...
                    if (null != value) {
                        // 驼峰转下划线风格，默认不对数字进行处理，即如果存在有数字的字段，请自行编写条件构造器
                        key = (CaseFormat.LOWER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, key));
                        // 注意，当前key为驼峰命名，对应的sql查询语句字段需要用as进行转换
